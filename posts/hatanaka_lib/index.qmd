...
Data servers hosting GNSS data in RINEX format typically employ Hatanaka-gzip compression to optimize storage. This is becoming [critical as networks and number of GNSS signals grow](../gnss_data_data_volume/index.qmd).
...
In programming languages like Python, these steps typically involve calling external tools using the [`subprocess` library](https://docs.python.org/3/library/subprocess.html), essentially executing external programs. These programs often store their output in intermediate files, which are then consumed by subsequent processing steps.

However, the necessity of parsing a text file (uncompressed RINEX, in step 4 above) to load it into memory significantly increases execution time. [While previous efforts have explored storing data in binary formats like Parquet](../parquet_format_for_gnss_measurements/index.qmd), these formats cannot beat the compression achieved by the highly specialized Hatanaka + gzip combination (`crx.gz` files).
...
Therefore, the main goal of this work is to deploy a software that:
...
The `hatanaka.to_dataframe` feature is currently unavailable on PyPI due to platform-dependent considerations. Deployment mechanisms are still under investigation.
...
1. *Direct*. This strategy utilizes the proposed refactoring for direct CRZ loading:
  1. Uncompress `gz` file and extract the `CRX` (Hatanaka compressed). This
  uses the built-in `gzip` library
  2. Direct read of the `CRX` file into a dataframe.

2. *System*. This approach implements the standard processing pipeline mentioned earlier, involving two steps:
  1. Use system call to `CRZ2RNX` to obtain the RINEX file
  2. Load the Rinex using the `py-roktools` rinex library

3. Pre-loaded *Parquet*: Assume that RINEX files have been pre-loaded and stored as `parquet` format, as proposed [in a previous work](../parquet_format_for_gnss_measurements/index.qmd). In this step, data is stored in binary format, therefore no parsing needed: just a simple load of a parquet file.
